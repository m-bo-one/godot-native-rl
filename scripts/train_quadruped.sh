...
INIT_FROM="${INIT_FROM:-}"               # warm-start from a source .zip (e.g. the walk net -> hurdles)
ENT_COEF="${ENT_COEF:-0.0}"              # PPO entropy coefficient; >0 aids exploration (hurdles)

INIT_ARG=()
[ -n "$INIT_FROM" ] && INIT_ARG=(--init_from "$INIT_FROM")
